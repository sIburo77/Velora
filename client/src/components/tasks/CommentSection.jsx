...
export default function CommentSection({ workspaceId, taskId }) {
  ...
  return (
    <div className="mt-4 border-t border-[var(--color-border)] pt-4">
      ...
      <div className="space-y-3 max-h-40 sm:max-h-60 overflow-y-auto mb-3">
        {comments.map((c) => (
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center flex-wrap gap-x-2 gap-y-0.5">
                ...
                {c.author_id === user?.id && (
                  <button
                    onClick={() => remove(c.id)}
                    className="sm:opacity-0 sm:group-hover:opacity-100 p-0.5 rounded hover:bg-red-500/10 text-content-muted hover:text-red-400 transition"
                  >
                    <Trash2 size={12} />
                  </button>
                )}
